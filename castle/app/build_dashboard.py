...
def build(instance, verbose=False, nav=False, scope=None):
    ...
    if month["estimated"]:
        # 原価率は週次の棚卸から出る（[[pnl.rate_at]]）。「直近の確定月の率」は在庫が
        # 効く前の説明で、2026-09-06まで冒頭に残っていた。人時単価は確定月のままでよい ──
        # 賞与が引当なので、月ごとの動きは1%に満たない（デモの16ヶ月で測った）。
        notice = ('<div class="notice"><b>当月は、会計の締めを待たずに出した着地です。</b>'
                  "売上と労働時間は日次の実績。原価率は週次の棚卸から（棚卸が届いていない先の日は"
                  "直近の週の率）。人時単価は直近の確定月（賞与は引当なので、月ごとにはほぼ動きません）。"
                  "</div>")
    else:
        notice = ""
    ...
    limits = [
        ...
        "<li><b>当月の原価率と人時単価の出どころ。</b>%s</li>" % metric["note"],
        ...
        "<li><b>粗利率は週の単位でしか動かない。</b>"
        "原価率は週次の棚卸から出すので、同じ週の日々は同じ率で並ぶ。"
        "棚卸が届いていない先の日は直近の週の率で、届いた時に置き換わる。</li>",
